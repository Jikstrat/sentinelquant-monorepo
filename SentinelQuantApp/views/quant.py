"""views/quant.py — Quant Dashboard view"""
import time
...
def render() -> None:
    ...
    def fetch_ticker_suggestions(query: str, market: str = "US Market") -> list[str]:
        ...
        Indian Market (NIFTY 50):
            Filters NIFTY50_TICKERS client-side â€” no network call needed.
        ...
        # â”€â”€ Indian Market â€” pure client-side filter, no HTTP request â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ US Market â€” Yahoo Finance live search â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if not query:
    ...
    INDICATOR_EXPLAINERS = {
        ...
        "RSI_signal": {
            "name": "RSI  â€”  Relative Strength Index (14)",
            "body": (
        ...
        "MACD_signal_trade": {
            "name": "MACD  â€”  Moving Average Convergence Divergence",
            "body": (
    ...
    # ---------------------------------------------------------------------------
    # CSS â€” clean, minimal dark theme, zero emojis
    # ---------------------------------------------------------------------------
    st.markdown("""
    <style>
    ...
    /* â”€â”€ Signal panel (below chart) â”€â”€ */
    .signal-panel {
        background: var(--surface); border: 1px solid var(--border);
    ...
    /* â”€â”€ Strategy how-it-works card â”€â”€ */
    .how-card {
        background: var(--surface); border: 1px solid var(--border);
    ...
    /* â”€â”€ Strategy reference cards â”€â”€ */
    .strat-card {
        background: var(--surface); border: 1px solid var(--border);
    ...
    /* â”€â”€ Empty state / feature cards â”€â”€ */
    .feat-card {
        background: var(--surface); border: 1px solid var(--border);
    ...
    live_html = (
        ...
        f"Live  Â·  refresh #{refresh_count}</span>"
    ...
    # ===========================================================================
    # STRATEGY FLASHCARDS  â€”  education panel shown before controls
    # ===========================================================================
    FLASHCARDS = [
        {
            "name":  "Moving Average",
            "desc":  "Detects trend direction by comparing two average price lines over time.",
            "rules": "SMA 20 > EMA 20  â†’  BUY\nSMA 20 < EMA 20  â†’  SELL",
        },
        {
            "name":  "RSI",
            "desc":  "Measures whether a stock is overbought or oversold on a 0â€“100 scale.",
            "rules": "RSI < 30  â†’  BUY  (oversold)\nRSI > 70  â†’  SELL  (overbought)",
        },
        {
            "name":  "MACD",
            "desc":  "Detects momentum changes by tracking two exponential moving averages.",
            "rules": "MACD crosses above signal  â†’  BUY\nMACD crosses below signal  â†’  SELL",
        },
        {
            "name":  "Bollinger Bands",
            "desc":  "Shows price volatility. Wide bands mean high volatility; narrow means low.",
            "rules": "Price near lower band  â†’  BUY\nPrice near upper band  â†’  SELL",
        },
        {
            "name":  "EMA Crossover",
            "desc":  "Compares a fast and a slow exponential average to spot momentum shifts.",
            "rules": "EMA 12 > EMA 26  â†’  BUY\nEMA 12 < EMA 26  â†’  SELL",
        },
    ]
    
    st.markdown(
        "<div class='flashcard-section-label'>Strategy Guide â€” understand each strategy before running analysis</div>",
        unsafe_allow_html=True,
    )
    
    # Row 1 â€” first 3 cards
    ...
    # Row 2 â€” last 2 cards (centred via empty padding columns)
    ...
    # ===========================================================================
    # CONTROL PANEL  â€”  single row, six columns, no sidebar
    # ===========================================================================
    ...
    with col1:
        # Single search input â€” label is the widget label, value drives suggestions.
    ...
    # ===========================================================================
    # ANALYSIS PIPELINE  â€”  runs on click AND every auto-refresh
    # ===========================================================================
    if st.session_state.analysis_run:
        ...
        # =======================================================================
        # SECTION 1  â€”  Performance Summary
        # =======================================================================
        st.markdown("---")
        best = comparison_df.iloc[0]
    
        sec(f"Performance Summary  â€”  best strategy: {best['Strategy']}")
        ...
        # =======================================================================
        # SECTION 2  â€”  Price Chart  (full width)
        # =======================================================================
        st.markdown("---")
        sec("Price Chart")
        ...
        # Strategy switcher â€” text buttons, no emojis
        if len(_sigs) > 1:
        ...
        # =======================================================================
        # SECTION 3  â€”  Backtesting Results
        # =======================================================================
        st.markdown("---")
        ...
        # pandas >= 2.1 renamed applymap â†’ map.
        ...
        for row in bt_rows:
            tl = row["_trade_log"]
            with st.expander(f"{row['Strategy']}  â€”  {len(tl)} trades", expanded=False):
                if tl.empty:
        ...
        # =======================================================================
        # SECTION 5  â€”  Strategy Reference
        # =======================================================================
        st.markdown("---")
    ...
    # ===========================================================================
    # EMPTY STATE  â€”  before first run
    # ===========================================================================
    else:
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        st.markdown("---")
